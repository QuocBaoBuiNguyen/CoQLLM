...
Mô tả CoQLLM có **HAI đường song song** đưa CF vào LLM và đóng góp = "kết hợp cả hai":
...
1. **✅ ĐÃ QUYẾT: Phương án (a) — BỎ HẲN đường CoRA ΔW khỏi phần method.** (Người dùng chốt 2026-08-11.) Mô tả đúng model thực = **chỉ soft-token bridge**. Cụ thể:
   ...
   - **ch2 §`ssec:cora-rel` (2.3.4):** GIỮ CoRA nhưng chỉ như **related work / một hướng khác** (tiêm trọng số) + là **baseline CoRA-MF**. Bỏ câu "CoQLLM hiện thực cơ chế CoRA như một trong hai đường".
...
5. Rà `references.bib`: `cite{cora2024}` vẫn giữ (CoRA là related work + baseline CoRA-MF), nhưng bỏ vai trò "cơ chế lõi của CoQLLM".
...
### A. Khung kiến trúc CoQLLM (cầu nối CF→LLM bằng Q-Former)
...
- **HFQFormerAdapter** (`src/coqllm/models/q_former/hf_qformer_adapter.py`, commit 14d8599, b0d61db): cầu nối **plain Q-Former (BLIP-2)** — learned queries + cross-attention vào CF. ⚠️ Code hiện *instantiate* module HF InstructBLIP Q-Former nhưng chạy với **instruction cố định** ⇒ về chức năng = BLIP-2 Q-Former (xem mục 0.6 — luận mô tả là plain Q-Former, KHÔNG InstructBLIP). text branch khởi tạo từ **bert-base-uncased**, `num_layers=4` (giảm 8→4, commit cfd6418), `num_queries=8`, `d_model=768`.
...
| **CoQLLM — user_conditioned (Run-1)** | 0.8147 | 0.5958 |
| **CoQLLM — + align_rank (Run-2)** | **0.8132** | **0.6062** |
