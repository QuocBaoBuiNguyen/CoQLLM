...
Chương này trình bày thiết lập thực nghiệm, kết quả đánh giá SigLLM so với các baseline,
...
Thực nghiệm được thực hiện trên hai tập dữ liệu chuẩn theo giao thức của
...
$34$ nghìn vật phẩm sách, với độ thưa và tỉ lệ người dùng/vật phẩm nguội cao hơn hẳn -- đây
chính là điều kiện để kiểm chứng cầu nối CF--LLM trên domain khó, nơi tín hiệu CF yếu và
LLM/Q-Former có nhiều dư địa đóng góp. Nhãn CTR được nhị phân hóa tương tự. Việc bổ sung
Amazon-Book cho phép đánh giá khả năng tổng quát của SigLLM ngoài domain phim và làm nổi
bật vai trò của mục tiêu bảo toàn thứ tự nội-người-dùng (Mục~\ref{ssec:align-rank}).

\subsection{Phân chia dữ liệu và giao thức đánh giá}
\label{ssec:phan-chia}

Theo giao thức của CoLLM~\cite{collm2023}, dữ liệu được sắp xếp theo thời gian và phân
...
\begin{itemize}
    ...
    theo giao thức này, có mặt trên cả hai tập dữ liệu.
\end{itemize}

\textbf{Baseline bổ sung:}
\begin{itemize}
    \item \textbf{CoRA}~\cite{cora2024}: đại diện hướng \textit{tiêm trọng số} (không soft
    token). Trên Amazon-Book, biến thể CoRA-MF được đưa vào bảng so sánh như một điểm tham
    chiếu cho hướng thiết kế khác với SigLLM (Mục~\ref{ssec:cora-rel}).
    ...
    Qwen2-7B-Base; \textit{lưu ý: SigLLM dùng Vicuna-7B-v1.5, khác họ backbone, nên so sánh
    ...
    cáo AUC/uAUC theo giao thức CTR, nên không thể so sánh trực tiếp về số liệu; được
    đề cập trong phần thảo luận}.
\end{itemize}

\textbf{Giao thức lấy số liệu baseline.} Trên MovieLens-1M, số liệu CoLLM/BinLLM lấy theo
giao thức CoLLM-parity. Trên Amazon-Book, số liệu CoLLM-MF/BinLLM/CoRA-MF lấy từ lần chạy
lại (re-run) trong CoRA~\cite{cora2024} để đảm bảo cùng phân chia dữ liệu. Cần lưu ý một
khác biệt giao thức: SigLLM chọn checkpoint tốt nhất theo \textbf{uAUC}, trong khi
CoLLM/CoRA chọn theo \textbf{AUC}; điều này được ghi chú trực tiếp ở bảng kết quả.

\subsection{Độ đo đánh giá}
\label{ssec:do-do}

Luận văn dùng hai độ đo chính theo giao thức CoLLM:
...
\begin{itemize}
    ...
    dùng rồi lấy trung bình. uAUC đánh giá chất lượng xếp hạng \textit{nội-người-dùng}
...
\begin{itemize}
    ...
    MF và LLM đông cứng. Không có tham số tiêm trọng số. Tổng số tham số huấn luyện được rất
...
Bảng~\ref{tab:ket-qua-chinh} và Bảng~\ref{tab:ket-qua-book} so sánh SigLLM với CoLLM,
BinLLM và các baseline trên MovieLens-1M và Amazon-Book theo AUC và uAUC.

\begin{table}[ht]
\caption{Kết quả trên MovieLens-1M (giao thức CoLLM-parity). SigLLM báo cáo hai cấu hình:
\textit{vanilla} (chỉ soft token) và \textit{+ user\_conditioned} (cấu hình đề xuất).
SigLLM chọn checkpoint theo uAUC.}
...
\hline
SigLLM (vanilla) & 0{,}7335 & 0{,}7009 \\
\hline
\textbf{SigLLM (+ user\_conditioned)} & \textbf{0{,}7475} & \textbf{0{,}6968} \\
\hline
\end{tabular}
\end{center}
\end{table}
...
trong CoRA~\cite{cora2024} (chọn checkpoint theo AUC); SigLLM chọn theo uAUC. uAUC tính
...
\hline
CoRA-MF~\cite{cora2024} (tiêm trọng số) & 0{,}8179 & 0{,}6262 \\
\hline
SigLLM (+ user\_conditioned, Run-1) & 0{,}8147 & 0{,}5958 \\
\hline
\textbf{SigLLM (+ align\_rank, Run-2)} & \textbf{0{,}8132} & \textbf{0{,}6062} \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\textbf{Amazon-Book.} Đây là domain thưa và khó hơn. Với \texttt{user\_conditioned} (Run-1),
SigLLM đạt AUC $0{,}8147$ và uAUC $0{,}5958$ -- \textit{vượt cả CoLLM-MF và BinLLM (theo lần
...
tổn thất này tác động vào thứ tự nội-người-dùng chứ không đụng khả năng phân biệt cross-user.
Con số $0{,}6062$ đưa SigLLM vào vùng cạnh tranh với CoRA-MF ($0{,}6262$), vốn dùng cơ chế
tiêm trọng số khác hẳn.

\textit{Lưu ý về giao thức so sánh.} SigLLM chọn checkpoint theo \textbf{uAUC}, còn
CoLLM/CoRA theo \textbf{AUC}; do đó uAUC của SigLLM là uAUC-tối-ưu-trên-frontier (lợi thế
...
tranh/vượt baseline dưới giao thức có kiểm soát'' thay vì đuổi theo một con số tuyệt đối.
...
học thứ tự xếp hạng \textit{trong} từng người dùng -- cặp vật phẩm nào người dùng này
...
mà ở \textit{chất lượng nhúng CF của MF cho người dùng/vật phẩm nguội}. Đây là căn cứ để
...
Bảng~\ref{tab:ablation} trình bày phân tích ablation các thành phần của SigLLM.
...
nên tổn thất xếp hạng nội-người-dùng phát huy tác dụng. Việc bật cơ chế theo từng tập dữ
liệu vì thế là một \textit{siêu tham số hợp lệ theo domain}, không phải sự bất nhất về
phương pháp.
...
thuần; (ii)~bật/tắt riêng biểu diễn đa token, tách projector, và conditioning InstructBLIP
dưới cùng backbone Vicuna; (iii)~ablation \texttt{align\_rank\_loss} trên MovieLens-1M (kỳ
...
\textit{ấm} (warm -- có nhúng MF, tức MF đã ``thấy'' người dùng/vật phẩm trong huấn luyện)
...
cho người dùng/vật phẩm nguội} chứ không phải bởi cầu nối: ở nhóm warm -- nơi MF cung cấp tín
hiệu tốt -- SigLLM khai thác hiệu quả và tiệm cận baseline mạnh nhất. Nhận định này khớp với
...
\subsection{Đối chiếu với ILM về giao thức đánh giá}
\label{ssec:doi-chieu-ilm}

ILM~\cite{ilm2024} không báo cáo AUC/uAUC theo giao thức CTR, nên không thể so sánh
số liệu trực tiếp. Đây là lý do chính luận văn đặt ra mục tiêu đánh giá Q-Former dưới
giao thức CoLLM: cung cấp cơ sở so sánh công bằng trong cùng điều kiện thực nghiệm.

\subsection{Điểm mạnh và hạn chế của cầu nối Q-Former}
\label{ssec:diem-manh-han-che}

So với lớp chiếu tuyến tính của CoLLM, cầu nối Q-Former có cơ chế trích chọn đặc trưng rõ
ràng hơn qua tập truy vấn học được và self-attention, và có khả năng mở rộng bằng
conditioning theo chỉ dẫn (InstructBLIP). Vì tín hiệu CF chỉ đi qua đường soft token, LLM
được giữ nguyên khi suy luận -- không có chi phí sinh trọng số theo từng mẫu như hướng tiêm
...
\begin{enumerate}
    ...
    tách projector, conditioning InstructBLIP, và \texttt{align\_rank\_loss} trên
    MovieLens-1M đều chưa được chạy dưới cùng backbone Vicuna; kết luận về từng thành phần
    vì thế còn hạn chế.
    ...
    khác giao thức chọn checkpoint theo uAUC của SigLLM.
    \item \textbf{Trần uAUC ở nhóm cold:} uAUC bị chặn trên bởi chất lượng nhúng MF cho
    người dùng/vật phẩm nguội (Mục~\ref{ssec:user-group}), không phải bởi cầu nối; cải
