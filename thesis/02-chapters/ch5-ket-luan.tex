...
soát -- có thể căn chỉnh tín hiệu CF vào không gian ngữ nghĩa của LLM hiệu quả
hơn lớp chiếu tuyến tính của CoLLM hay không? Để trả lời, luận văn thiết kế, hiện thực và đánh
giá mô hình SigLLM, đóng góp theo hai hướng đã phát biểu ở Mục~\ref{sec:dong-gop}:

\textbf{Đóng góp 1: Cầu nối Q-Former được thích nghi và tăng cường cho bài toán gợi ý.}
SigLLM thay thế lớp MLP một lớp của CoLLM bằng kiến trúc Q-Former, trong đó tín hiệu CF
(nhúng người dùng, vật phẩm mục tiêu, lịch sử) thay thế đặc trưng ảnh làm đầu vào
...
nội-người-dùng -- cùng các lựa chọn thiết kế (biểu diễn đa token, backbone đã tinh chỉnh chỉ
dẫn, tách lớp chiếu người dùng/vật phẩm) và khảo sát InstructBLIP. Kết quả chi tiết được
trình bày ở Chương~\ref{Chapter4}.

\textbf{Đóng góp 2: Đánh giá có hệ thống dưới giao thức AUC/uAUC trên hai tập dữ liệu.}
Luận văn đánh giá cầu nối Q-Former dưới giao thức CTR với AUC và uAUC -- cùng giao thức với
...
\begin{itemize}
    ...
    \item \textbf{Amazon-Book -- cạnh tranh/vượt baseline trên domain khó.} Với
    \texttt{user\_conditioned} (Run-1), SigLLM đạt $0{,}8147$/$0{,}5958$, vượt CoLLM-MF và
    ...
    $0{,}6238$ vs $0{,}5258$), cho thấy đòn bẩy cải thiện tiếp theo nằm ở chất lượng nhúng CF
    cho người dùng/vật phẩm nguội (cold).
\end{itemize}
...
\begin{enumerate}
    ...
    projector, InstructBLIP, \texttt{align\_rank} trên MovieLens-1M), đây là phần thực
    nghiệm còn để ngỏ.
    ...
    khác SigLLM chọn theo uAUC) -- giới hạn khả năng so sánh thuần về cơ chế căn chỉnh.

    \item \textbf{Trần uAUC ở nhóm cold:} uAUC bị chặn trên bởi chất lượng nhúng MF cho
    người dùng/vật phẩm nguội, không phải bởi cầu nối.
\end{enumerate}
...
\begin{itemize}
    \item Đánh giá trên Amazon Books và các tập dữ liệu ngoài domain phim để kiểm chứng
    khả năng tổng quát.
...
\begin{itemize}
    \item Ablation bật/tắt riêng InstructBLIP conditioning để chốt đóng góp độc lập
    của thành phần này.
    \item Phân tích attention map của Q-Former để hiểu truy vấn nào học cách trích xuất
    thông tin gì từ tín hiệu CF.
\end{itemize}

\textbf{Cải thiện uAUC:}
\begin{itemize}
    \item Tiền huấn luyện MF trên dữ liệu dày đặc hơn (nhiều tháng hơn), đặc biệt cho
    nhóm người dùng/vật phẩm nguội -- đòn bẩy được phân rã warm/cold chỉ ra là trần thực sự
    của uAUC.
    \item Mở rộng mục tiêu bảo toàn thứ tự (\texttt{align\_rank\_loss}, đã hiệu quả trên
    Amazon-Book) sang các domain khác và khảo sát trọng số $\lambda$ tối ưu theo từng tập.
\end{itemize}
...
\begin{itemize}
    ...
    \item Khảo sát hướng tiêm trọng số kiểu CoRA như một đường \textit{bổ sung} cho soft
    token (chưa hiện thực trong luận văn này) để so sánh trực tiếp hai cơ chế đưa CF vào LLM.
