...
``coqllm.datasets.qformer.qformer_loader``.
...
Usage
-----
    python -m coqllm.pipelines.multimodal.build_qformer_dataset \
...
from coqllm.datasets.qformer.qformer_alignment_builder import QFormerAlignmentBuilder


LOGGER = NotebookLogger.rich_logger("coqllm.build_qformer_dataset")
...
def build_qformer_pkls(cfg) -> None:
    ...
    ----------
    cfg
        Parsed ``coqllm.common.config.Config`` object. Must expose
    ...
    # D2: domain noun for the Q-Former instruction templates (movie / book).
