...
from coqllm.common import NotebookLogger
from coqllm.datasets.base.rec_base_dataset_builder import RecBaseDatasetBuilder
from coqllm.datasets.qformer.qformer_alignment_dataset import QFormerAlignmentDataset

LOGGER = NotebookLogger.rich_logger("coqllm.qformer_alignment_builder")
...
class QFormerAlignmentBuilder(RecBaseDatasetBuilder):
    ...
    # D2 fix — the domain noun is a PARAMETER, not hardcoded "movie". These
    # templates were literally telling the Q-Former "this movie" while
    # processing Amazon books. They live here as the single source of truth and
    # render via `render_templates`. `{meta}` promises "title and genres" only
    # when genres carry signal (Amazon-Book ships a CONSTANT synthetic genres
    # field, so promising "genres" there teaches nothing — D3).
    ...
    def render_templates(cls, which, item_noun=None, has_genres=True):
        ...
        any downstream stage on the SAME (item_noun, has_genres) pair — the
        Q-Former is pretrained on whatever wording Stage 1 used, so changing it
        afterwards creates a train/inference mismatch and requires re-running
        Stage 1 + Stage 2.
        """
    ...
    def build_qformer_alignment_samples(
        ...
        # D3: emit genres only when the field actually varies. Amazon-Book ships
        # a CONSTANT synthetic genres value (". Genres: Books." on every row) —
        # trivially predictable, so it dilutes the ITG / Stage-2 objective and
        # flatters the loss without teaching content.
        ...
        # D2: render the instruction templates for this domain (movie / book),
        # dropping the genres promise when genres carry no signal (has_genres).
        templates = {
            k: QFormerAlignmentBuilder.render_templates(k, item_noun, has_genres)
            for k in ("item_text", "item_item", "user_item")
        }
        log_step(
            "Q-Former instruction domain",
