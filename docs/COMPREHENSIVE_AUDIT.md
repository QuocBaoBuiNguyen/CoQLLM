# CoQLLM Comprehensive Experiment Audit
...
|---|---|---|---|---|---|---|
| 1 | **Vanilla Q-Former** (instruction-aware OFF) | `feat/best-0.72-warm` (commit 13702bb) | `CoQLLM_vanilla_29_5_2026.ipynb` | **0.7081** (epoch 5) | 0.7213 | ✓ |
| 2 | **Instruction-aware V1** (12 paraphrase) | `feat/ablation-instruction-aware` (a69e9e7) | `CoQLLM._best_24_05_2026ipynb.ipynb` + `CoQLLM_rerun_28_5_2026.ipynb` | **0.7098** (24/05 best) / 0.7018 (28/05 rerun) | 0.7265 | ✓ |
...
| 4 | **Interaction-aware Q-Former** | `feat/interaction-aware-qformer` (7d8279d) | `CoQLLM_interaction_instruction.ipynb` | **0.7036** | n/a | ✓ |
| 5 | **User-conditioned queries** | `feat/user-conditioned-queries` (2478f3f) | `CoQLLM.ipynb` | **0.7060** (ep3) / 0.7152 (07-20 rerun) | 🛑 test 0.7086/0.7287 từ eval path nghi vấn — corrected `eval_test.py`: no collapse, uAUC vẫn ≤ baseline (xem warning §2) | ✓ |
...
| 25 | **Stage 1 loss ablation** (skip ITG/ITM/UI/II individually) | Implicit | ✗ NOT TESTED on CoQLLM (ILM paper Table 4 has this on OpenP5, không transfer được số) |
...
> 🛑 **TEST COLUMNS UNRELIABLE (2026-07-22) — re-verify before citing.**
> A code-verified eval bug was found: the Stage-3 Step-2 training script with
> `run.evaluate=True` evaluates the **wrong weights** (Step-1 LoRA + Stage-2
> Q-Former, NOT the trained Step-2 model — `apply_step2_overrides` forces
> `model.ckpt=Step-1` and `evaluate(skip_reload=True)` never loads the Step-2
> checkpoint). **Any Test AUC/uAUC/warm/cold/score-gap column below that was
> produced via that path is invalid.** This includes the doubted **Vanilla
> baseline "0.7325" test AUC** and the user-conditioned test row — their
> provenance is not yet confirmed. The canonical correct method is now
> `eval_test.py` (loads Step-1 LoRA, overlays the trained Step-2 checkpoint
> `strict=False`). A corrected Run-1 re-eval shows the user-conditioned
> "calibration collapse" does NOT reproduce (pred_pos ≈ 0.61 not 0.78) → that
> collapse was likely an eval artifact. **Action: re-run every model's test
> splits through `eval_test.py` and replace the columns.** Until then, treat all
> Test* cells as provisional.
...
| **User-conditioned queries** | `queries += user_proj(user_cf)` | 79M | same | same | 2-step | **3** | **0.7582** ← max AUC | 0.7060 | 0.5980 | 0.7287† | 0.7086† | 0.7410† | 0.7210† | 0.7057† | 0.6492† | 0.134† | 🛑 †Test cols from suspect eval path — see warning above. Corrected `eval_test.py` Run-1: no collapse (pred_pos 0.61), test uAUC ~0.705/0.716 still ≤ baseline |
...
Đây là **bottleneck major** trong audit. CoLLM paper dùng `Linear → GELU → Linear` (intermediate 10x). CoQLLM **không có baseline này** để compare.

| Question | Answer |
|---|---|
| File implement? | KHÔNG có (`grep -rn "MLP" src/coqllm/models/ → empty`) |
...
**Status: KHÔNG có ablation đầy đủ Stage 1/2 trên CoQLLM.**

| Ablation | Tested trên CoQLLM? | Source |
|---|---|---|
...
| Bỏ Stage 1 ITG | ✗ NOT TESTED on CoQLLM. ILM Table 5 trên OpenP5 ML1M: ILM-IT-UI eval loss 4.07 vs ILM-IT 4.17 → UI giúp |
| Bỏ Stage 1 ITM | ✗ NOT TESTED |
| Bỏ Stage 1 UI loss | ✗ NOT TESTED on CoQLLM. ILM Table 4 chỉ test trên rec retrieval metric, không transfer được số |
| Bỏ Stage 1 II loss | ✗ NOT TESTED on CoQLLM. ILM Table 4 same situation |
...
→ **Đáng chạy:** Stage 3 từ random Q-Former → đo "Stage 1+2 contribution to uAUC" trên CoQLLM specifically.
...
`src/coqllm/tasks/base/rec_base_task.py:196-200`:
```python
all_results = {
...
|---|---|---|
| User-conditioned queries | +0.036 (val) → **−0.004 (test)** | −0.004 (val) / −0.008 (test) |
...
**⚠️ Test-set caveat (user-conditioned, 2026-07-20):** cái +0.036 val_AUC là **overfitting** — trên test AUC sập về 0.7287 (< vanilla 0.7325) và calibration hỏng (pred_pos_rate@0.5 → 0.78–0.83, score_gap 0.229→0.13). Per-user additive bias triệt tiêu within-user (uAUC hòa) nhưng thành nhiễu cross-user global (AUC/calibration tụt) trên test users chưa thấy. Chi tiết: `ABLATION_RESULTS.md` §"Test-set results (2026-07-20 rerun)".
...
```bash
grep -rn "grad_norm\|grad\.norm" src/coqllm/ → empty
```
...
| **CoQLLM Vanilla** | Q-Former + LoRA | **0.7325** | **0.7170** | This work |
| **CoQLLM Instruction-aware** | Q-Former + LoRA + instruction | 0.7322 | 0.7131 | This work |
| **CoQLLM User soft tokens** | Q-Former + user channel + LoRA | 0.7336 | 0.7145 | This work |
| **CoQLLM + CoRA (bundle)** | Q-Former + multi-token CF + CoRA weight-delta + Qwen2-Instruct | **0.7508** | 0.7041 | This work (cai-tien-va-ket-qua.md) |

→ **Cùng split, cùng test set** (CoLLM/BinLLM/SellaRec dùng cùng OOD2 split).

→ **CoQLLM Vanilla tied với CoLLM-MF** (uAUC 0.7170 vs 0.7179), below BinLLM (-0.011 uAUC) và SellaRec (-0.033 uAUC).

→ **CoRA bundle: AUC cao nhất trong các biến thể CoQLLM (0.7508, +0.018 vs Vanilla, trên cả CoLLM-MF và BinLLM về AUC) nhưng uAUC lại THẤP nhất (0.7041, −0.013 vs Vanilla).** AUC↑ uAUC↓ là dấu hiệu CoRA khuếch đại phân biệt cross-user (population-level) chứ không phải xếp hạng nội-user. Warm/cold breakdown (`cai-tien-va-ket-qua.md`): test_warm AUC 0.7830 / uAUC 0.7145; test_cold AUC 0.7039 / uAUC 0.6372 — uAUC tổng bị cold (51/224 user) kéo xuống.
...
**No MLP-bridge baseline implemented on CoQLLM internally** — chỉ có CoLLM-MF từ paper.
...
- ✓ External comparison: CoQLLM (Q-Former) tied CoLLM-MF (MLP) → suggest Q-Former không hơn MLP đáng kể
...
- CoQLLM-specific ablation: ✗ NOT TESTED

→ **Cannot quantify Stage 1/2 contribution on CoQLLM final uAUC.** Worth running Stage 3 from random Q-Former để measure.
...
6. **Stage 1/2 contribution chưa quantify** trên CoQLLM-specific.
